...
def detail(profession,id):
    ...
    if profession=="Patient":
        ...
        return render_template('detail.html',user=user,id=id,profession=profession,appointment_details=appointment_details,treatment_detail=treatment_detail)
    ...
    if result>0:
        user = cur.fetchone()
    return render_template('detail.html',user=user,profession=profession,id=id)
...
def profile(profession):
    ...
    email=session['email']
    if profession=="Patient":
        cur.execute("Select pid,Name,Email,Contact,SEX from Patient Where email=%s",[email])
        user=cur.fetchone()
        id = user['pid']
    elif profession=="Doctor":
        cur.execute("Select did,Name,Email,Username,SEX from Doctor Where email=%s",[email] )
        user=cur.fetchone()
        id=user['did']
    elif profession=="Nurse":
        cur.execute("Select nid,Name,Email,Username,SEX from Nurse Where email=%s",[email])
        user = cur.fetchone()
        id = user['nid']
    elif profession=="Receptionist":
        cur.execute("Select rid,Name,Email,Username,SEX from Receptionist Where email=%s",[email])
        user = cur.fetchone()
        id = user['rid']
    else:
        pass
